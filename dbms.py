...
# Note: Aiven Cloud MySQL typically requires SSL connections.
DB_CONFIG = {
...
def get_db_connection():
    try:
        # Aiven requires SSL. Try connecting with basic SSL enabled.
        return mysql.connector.connect(
            **DB_CONFIG,
            ssl_ca=""  # Empty string often forces mysql-connector to use default certs if supported
        )
    except Exception as e:
        try:
            # Fallback configuration if standard SSL args fail
...
# --- 5. MAIN APPLICATION CONTENT ---
    ...
    if conn:
            ...
            with t3:
                if st.session_state.user_role == 'admin':
                    with st.form("edit_dish_form"):
                        ...
                        e_avail = st.selectbox("Status", options=[1, 0], format_func=lambda x: "Available" if x == 1 else "Out of Stock")
        ...
        # ==========================================
            ...
            with t2:
                ...
                if st.button("Generate Invoice & Checkout"):
                    if not inv_phone:
                        st.warning("Please enter the customer's phone number!")
                    elif inv_table_id is None:
                        st.error("Cannot process checkout without a valid table selection.")
                    else:
                        try:
                            conn.start_transaction()
        ...
        # Always safely terminate active connection instances
        conn.close()
